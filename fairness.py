...
def model_performance(model,test_x,test_y,train_x,train_y,all_test,all_train, target_feature ,protected_groups, reg=False):
    ...
    if reg:
        ...
        for pg in protected_groups.keys():
            ...
            result_sum_test=result_sum_test.append(Result_sum(DF_test,pg))
            result_sum_train=result_sum_train.append(Result_sum(DF_train,pg))
...
def fairness(models,x,y,protected_groups={},metric="DI", threshold=0.8, xextra=False,reg=False):
    '''
    Parameters
    ----------
    Model: list of models,model object, can be sklearn, tensorflow, or keras
